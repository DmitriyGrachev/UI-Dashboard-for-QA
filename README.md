...
### CPU benchmark: один запуск, один архів

На Linux можна автоматично зібрати результати без ручного копіювання
`docker stats`. Потрібні Python 3 (лише стандартна бібліотека), Docker та вже
завантажений образ `maven:3.9-amazoncorretto-21`. Скрипт не запускає Docker daemon
і не завантажує образ автоматично.

Перевірити план без Docker, credentials або звернень до B2:

```bash
python3 scripts/b2_cpu_benchmark.py --dry-run
```

Запускати **лише з ізольованого benchmark checkout**, не з production-каталогу:

```bash
cd /opt/dataox/validator-b2-benchmark
python3 scripts/b2_cpu_benchmark.py \
  --env-file /opt/dataox/validator-b2-benchmark.env
```

Env-файл має бути доступний тільки власнику (`chmod 600`). Він повинен містити
`B2_ENDPOINT`, `B2_BUCKET`, `B2_ACCESS_KEY_ID`, `B2_SECRET_ACCESS_KEY` і непорожній
`B2_OBJECT_PREFIX`, наприклад `validator/server-benchmark`. Значення не потрібно
передавати в командному рядку. У live-режимі цей запуск **справді завантажує
тестові об'єкти у B2**.

Сценарій спочатку готує Java-тести без B2 credentials, потім збирає 60 секунд
базових метрик. Далі порівнює Sync із конкурентністю 8, 16 і 32 при квоті 2 CPU:
по 3 повтори, 300 файлів у кожному. Наостанок перевіряє квоту 1 CPU з найменшою
конкурентністю, яка досягла хоча б 90% найкращої зваженої швидкості першого
порівняння. Це евристика вибору наступного тесту, а не автоматична рекомендація
для production. Ліміти тестового контейнера: 3 GiB RAM, без додаткового swap,
512 PIDs. Production-контейнери, їхні налаштування, БД та каталог PNG не змінюються.

Номінально одночасно зберігається приблизно **444 MB тестових даних**
(300 файлів + один warmup); за всі 12 повторів відправляється близько 5.33 GB.
Це не гарантія дотримання cap: повторні PUT можуть створювати версії. Перевірте
запас у bucket до запуску. Після кожного повтору існуючий Java-стенд видаляє
лише власний унікальний підпрефікс і перевіряє відсутність версій/delete markers.
Якщо тест або перевірка cleanup неуспішні, наступна конфігурація не запускається.

Для окремого порівняння **4 та 8 потоків лише при 2 CPU**, без додаткового
тесту на 1 CPU:
...
Це 6 прогонів (по 3 на кожне значення), номінально близько **2.66 GB**
відправлених даних та тих самих **444 MB** одночасного зберігання без ретраїв.
`--concurrency` приймає різні значення з `4 8 16 32`; без нових параметрів
попередній сценарій не змінюється. Додайте `--dry-run`, щоб перевірити план
і бюджет без читання credentials, Docker або B2.

Шлях до підсумкового `.tar.gz` друкується наприкінці. Він містить короткий звіт,
метрики з UTC-часом, очищені від credentials логи та профілі Java Flight Recorder.
Маркери фаз відокремлюють startup/warmup, вимірюване завантаження, перевірку
версій та cleanup. Середня швидкість обчислюється як сумарні успішні файли,
поділені на сумарний час вимірюваного завантаження, а не як середнє files/s.

CPU/RAM/network фіксуються для контейнерів, CPU/RAM — також для Linux host.
Лічильники CPU throttling збираються там, де cgroup це дозволяє; відсутні
метрики позначаються як недоступні, а не як нуль. Docker CPU 100% означає
приблизно одне логічне ядро. Квота `--cpus` обмежує CPU-час, але не закріплює
окремі ядра та не ізолює мережу чи диск; див.
[обмеження ресурсів Docker](https://docs.docker.com/engine/containers/resource_constraints/#cpu).
Вибіркові CPU-вимірювання можуть пропустити короткі піки; за ними не можна
довести, що latency recognition API не змінилася.

JFR записується для JVM, яка виконує тест, а не Maven JVM. Профіль дозволяє
лише події CPU/стеків, GC/JIT та часу I/O; environment, system properties,
аргументи процесів, повідомлення exceptions і heap dumps не записуються.
Env-файл і Surefire XML не входять до архіву. У профілі залишаються технічні
дані (шляхи, назви класів/потоків та адреси з'єднань), тому передавайте його
тільки довіреному отримувачу.

При `Ctrl+C` сценарій зупиняє тільки власний тестовий контейнер та зберігає
неповний звіт. Примусова зупинка не гарантує B2 cleanup: перевірте вказаний у
логах унікальний префікс перед наступним запуском. Після копіювання архіву
можна аналізувати його локально — доступ до production або до B2 не потрібен.

Офлайн-перевірка самого збирача, без Docker/B2:

```bash
python3 -m unittest discover -s scripts -p 'test_b2_cpu_benchmark.py'
```
