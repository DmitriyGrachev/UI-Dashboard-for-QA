...
- `200 image/png` — поток PNG из локального каталога или B2, без загрузки всего
  файла в память и без раскрытия B2 URL вызывающему сервису.
...
- `404` — изображение отсутствует локально и в B2.
- `503` — временная ошибка хранилища; это не доказательство удаления PNG.

Integration API скрывает выбор источника: локальный файл читается через файловый
`InputStream`, облачный — через S3 `GetObject`, после чего Validator передаёт тот же
поток в HTTP response. Успешный ответ содержит `Content-Type: image/png`,
`Content-Length`, исходное имя в `Content-Disposition` и `Cache-Control: no-store`.
Обычный операторский `/api/images/{imageId}/content` не меняется и для старого
cloud-backed PNG по-прежнему возвращает `307` напрямую в B2.
...
Подставьте реальный HTTPS-домен и ID. При `200` `screenshot.png` уже содержит
готовый PNG независимо от фактического источника. Следовать redirect и хранить
B2 credentials в сервисе Игоря не требуется.
